...
namespace Microsoft.AspNetCore.SignalR.NetClient.Transports.WebSockets
{
    internal static class WebSocketMessageReader
    {
        ...
        public static async Task<WebSocketMessage> ReadMessageAsync(WebSocket webSocket, int bufferSize, int? maxMessageSize, CancellationToken disconnectToken)
        {
            ...
            WebSocketReceiveResult receiveResult = await TaskAwaiterHelper.PreserveCultureNotContext(webSocket.ReceiveAsync(_emptyArraySegment, disconnectToken));
            ...
            receiveResult = await TaskAwaiterHelper.PreserveCultureNotContext(webSocket.ReceiveAsync(arraySegment, disconnectToken));
