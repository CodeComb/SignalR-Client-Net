...
namespace Microsoft.AspNetCore.SignalR.NetClient.Transports
{
    public class LongPollingTransport : HttpBasedTransport
    {
            ...
            lock (_stopLock)
            {
                ...
                .ContinueWith(task =>
                {
                    ...
                    else
                    {
                        try
                        {
                            next = TaskAsyncHelper.Then(task.Result.ReadAsString(readBuffer => OnChunk(connection, readBuffer)), raw => OnMessage(connection, raw));
                        }
                        catch (Exception ex)
                        {
                            exception = ex;
                    ...
                    TaskAsyncHelper.Finally(next, state => TaskAsyncHelper.Then(OnAfterPoll((Exception)state), () => Poll(connection, connectionData)),
                        exception);
                });
            }
        }
        ...
        private void TryDelayedReconnect(IConnection connection, ThreadSafeInvoker reconnectInvoker)
        {
            if (IsReconnecting(connection))
            {
                ...
                TaskAsyncHelper.Then(TaskAsyncHelper.Delay(ReconnectDelay), () => TryReconnect(connection, reconnectInvoker));
            }
        }
