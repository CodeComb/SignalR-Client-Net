...
namespace Microsoft.AspNetCore.SignalR.NetClient.Tests.Transports
{
    public class ClientTransportBaseTests
    {
        [Fact]
        ...
        public void CannotNegotiateUsingFinishedTransport()
        {
            ...
            Assert.Equal(
                ...
                (string) Assert.ThrowsAsync<InvalidOperationException>(
                    () => transport.Negotiate(Mock.Of<IConnection>(), "connectionData")).Result.Message);
        }
        ...
        public void CannotInvokeProcessResponseBeforeStartingTransport()
        {
            Assert.Equal(
                ...
                (string) Assert.Throws<InvalidOperationException>(
                    () => new Mock<ClientTransportBase>(Mock.Of<IHttpClient>(), "fakeTransport") { CallBase = true }.Object
